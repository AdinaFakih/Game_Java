...
public class SimpleGame {
	...
	public void run(int choose) throws Exception {
		SimpleGame simplegame = new SimpleGame();
		if (choose == 1) {
			System.out.println("Enter time, in seconds to hours, minutes, and seconds:\n");
			Scanner sc = new Scanner(System.in);
			int num = sc.nextInt();
			String time = simplegame.convertTime(num);
			System.out.println("Time:" + time);
			sc.close();
		} else if (choose == 2) {
			System.out.println("Enter an integer to calculate the sum of all digits in it:\n");
			Scanner sc = new Scanner(System.in);
			int num = sc.nextInt();
			int sum = simplegame.digitsSum(num);
			System.out.println("Sum:" + sum);
			sc.close();
		} else {
			throw new Exception("You choose wrong num");	
		}
	}
	...
	public static void main(String[] args) {
		...
		System.out.println("Which game do you want do play? \n 1. Convert time, from seconds to hours, minutes, and seconds \n 2. Calculate the sum of all digits in an integer");
		Scanner sc = new Scanner(System.in);
		int choose = sc.nextInt();
		System.out.println("Choose: " + choose);
		
		try {
			simplegame.run(choose);
		} catch (Exception e) {
